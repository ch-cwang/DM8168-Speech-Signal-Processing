...
Void Server_init(Void) {
    if (Module_curInit++ != 0) return;
    Registry_addModule(&Registry_CURDESC, MODULE_NAME);
}

Void Server_exit(Void) {
    if (Module_curInit-- != 1) return;
}
...
Int Server_create(UInt16 remoteProcId) {
    ...
    /* 【极度关键】：既然给了 BLOCK_COUNT 个写出额度，就必须预先填入对应的空闲车位号 (Index)，
       否则 DSP 第一次写出时就会取到随机垃圾值，导致覆盖未知物理内存！ */
    ...
    Notify_registerEvent(Module.remoteProcId, Module.lineId, Module.eventId, Server_notifyCB, (UArg)&Module);
    do {
        status = Notify_sendEvent(Module.remoteProcId, Module.lineId, Module.eventId, APP_CMD_NOP, TRUE);
        if (status == Notify_E_EVTNOTREGISTERED) Task_sleep(100);
...
Int Server_exec() {
    ...
    Notify_sendEvent(Module.remoteProcId, Module.lineId, Module.eventId, APP_SPTR_ADDR_ACK, TRUE);
    ...
    while (g_running) {
        ...
        if (!g_running) break; /* 被关机信号踹醒，立即跳出 */
        ...
        Notify_sendEvent(Module.remoteProcId, Module.lineId, Module.eventId, 
            (CMD_SERVER_TO_APP_RECORD_DONE << 16) | active_rx_idx, TRUE);
            
        /* 告诉 Host：这块播放我已经算好了，你赶紧拿去播吧 */
        Notify_sendEvent(Module.remoteProcId, Module.lineId, Module.eventId, 
            (CMD_SERVER_TO_APP_DATA_READY << 16) | active_tx_idx, TRUE);
    }
...
/* ========================================================================== */
Void Server_notifyCB(UInt16 procId, UInt16 lineId, UInt32 eventId, UArg arg, UInt32 payload) {
    ...
    /* 【极度关键的防死锁机制】：拦截 Host 的关机命令。
       若此时 DSP 主任务卡在 full_in 或 empty_out 等待数据，如果不手动 post 信号量，
       主任务将永远无法醒来看到 g_running=0 的变化，从而沦为僵尸进程卡死整板！*/
...
Int Server_delete() {
    ...
    event = Server_waitForEvent(&Module.eventQueue);
    if (event >= APP_E_FAILURE) return -1;

    /* 发送关机 ACK 确认，完成完美闭环的四次挥手 */
    Notify_sendEvent(Module.remoteProcId, Module.lineId, Module.eventId, APP_CMD_SHUTDOWN_ACK, TRUE);
    Notify_unregisterEvent(Module.remoteProcId, Module.lineId, Module.eventId, Server_notifyCB,(UArg) &Module);
